...
// !Заголовочное
constexpr size_t realAllocatorMetaSize = sizeof(std::pmr::memory_resource*) + sizeof(allocator_with_fit_mode::fit_mode) + sizeof(size_t) + sizeof(std::mutex) + sizeof(void*);;
        ...
        allocator_with_fit_mode::fit_mode allocate_fit_mode) : _trusted_memory(nullptr)
{
    ...
    if (space_size < allocator_metadata_size + occupied_block_metadata_size)
    {
        throw std::bad_alloc();
    }
